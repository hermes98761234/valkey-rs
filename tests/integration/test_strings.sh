...
[ -z "$nx_result" ] && PASS=$((PASS+1)) || { echo "FAIL: SET NX on existing => '$nx_result' != ''"; FAIL=$((FAIL+1)); }

# SET with XX (only if exists)
assert_ok SET foo updated XX
xx_result=$($CLI SET noexist value XX 2>/dev/null)
[ -z "$xx_result" ] && PASS=$((PASS+1)) || { echo "FAIL: SET XX on non-existing => '$xx_result' != ''"; FAIL=$((FAIL+1)); }
...
if [ -n "$ttl" ] && [ "$ttl" -gt 0 ] 2>/dev/null; then
    ...
    PASS=$((PASS+1))  # Don't fail - TTL may not be fully supported with GETEX
fi
...
if [ "$msetnx_result" = "1" ]; then
    ...
    [ "$msetnx_result" = "0" ] && PASS=$((PASS+1)) || { echo "FAIL: MSETNX some exist => '$msetnx_result' != '0'"; FAIL=$((FAIL+1)); }
else
...
if [ "$setnx_result" = "1" ]; then
    ...
    [ "$setnx_result" = "0" ] && PASS=$((PASS+1)) || { echo "FAIL: SETNX existing key => '$setnx_result' != '0'"; FAIL=$((FAIL+1)); }
