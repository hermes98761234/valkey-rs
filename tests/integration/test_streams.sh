...
[ -n "$id1" ] && ((PASS++)) || { echo "FAIL: XADD returned empty"; ((FAIL++)); }

id2=$($CLI XADD mystream "*" name "Bob" age "25" 2>/dev/null)
[ -n "$id2" ] && ((PASS++)) || { echo "FAIL: XADD second entry returned empty"; ((FAIL++)); }
...
[ -n "$xrg_result" ] && ((PASS++)) || { echo "FAIL: XREADGROUP returned empty"; ((FAIL++)); }
...
[ -n "$pending" ] && ((PASS++)) || { echo "FAIL: XPENDING returned empty"; ((FAIL++)); }

# XDEL
assert_eq "1" XDEL mystream "$id1"

# XTRIM
assert_ok XADD trimstream "*" k "v1"
assert_ok XADD trimstream "*" k "v2"
assert_ok XADD trimstream "*" k "v3"
assert_eq "2" XTRIM trimstream MAXLEN 2
assert_eq "2" XLEN trimstream

# XINFO STREAM
info=$($CLI XINFO STREAM mystream 2>/dev/null)
[ -n "$info" ] && ((PASS++)) || { echo "FAIL: XINFO STREAM returned empty"; ((FAIL++)); }

# XINFO GROUPS
groups=$($CLI XINFO GROUPS mystream 2>/dev/null)
[ -n "$groups" ] && ((PASS++)) || { echo "FAIL: XINFO GROUPS returned empty"; ((FAIL++)); }
